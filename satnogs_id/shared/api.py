...
class SatnogsClient:
    ...
    def __init__(self, cache_dir: str | Path | None = None, min_interval: float = 0.4,
                 max_retries: int = 6, backoff: float = 8.0) -> None:
    ...
    def _raw(self, url: str, auth: bool, accept: str = "application/json") -> tuple[bytes, Any]:
    ...
    def observations(self, *, norad: int | None = None, sat_id: str | None = None,
                     max_pages: int = 1, **filters: Any) -> list[dict]:
        ...
        url = f"{self.NETWORK}/observations/?" + "&".join(f"{k}={v}" for k, v in params.items())
        out: list[dict] = []
    ...
    def download(self, url: str, dest: str | Path) -> Path:
        ...
        try:
            ...
            req = urllib.request.Request(url, headers={"Authorization": f"Token {settings.db_key}"})
    ...
    def celestrak_gp_tle(self, intdes: str) -> list[tuple[str, str, str]]:
        ...
        body, _ = self._raw(f"{self.CELESTRAK_GP}?INTDES={intdes}&FORMAT=tle", auth=False, accept="text/plain")
        lines = [ln.rstrip() for ln in body.decode().splitlines() if ln.strip()]
        return [(lines[i], lines[i + 1], lines[i + 2]) for i in range(0, len(lines) - 2, 3)]


def nearest_tle(observations: Iterable[dict], target_date: str) -> tuple[str, str, str] | None:
